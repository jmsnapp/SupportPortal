...
{
    public class IntegrationTypeRepository : IGenericRepository<IntegrationType, IntegrationType, IntegrationTypeEntity>
    {
        ...
        public IntegrationTypeRepository(SupportPortalDBContext context)
        {
            _context = context ?? throw new ArgumentNullException(nameof(context));

        }

        public async Task<List<IntegrationType>> GetAllAsync(CancellationToken ct)
        {
            List<IntegrationType> lstReturn = new List<IntegrationType>();

            // Ensure connection is open
            using var command = _context.Database.GetDbConnection().CreateCommand();
            command.CommandText = "dbo.uspGetAllIntegrationTypes";
            ...
            lstReturn = GetIntegrationTypeListFromDataReader(result);

            return lstReturn;

        }

        public async Task<List<IntegrationType>> GetAllActiveAsync(CancellationToken ct)
        {
            List<IntegrationType> lstReturn = new List<IntegrationType>();
        ...
        public Task<List<IntegrationType>> GetByParentIdAsync(Int64 parentId, CancellationToken ct)
        {
            throw new NotImplementedException("Parent property not valid for this object.");

        }

        public async Task<IntegrationType> GetByIdAsync(Int64 id, CancellationToken ct)
        {
            IntegrationType objReturn = new IntegrationType();

            // Ensure connection is open
            using var command = _context.Database.GetDbConnection().CreateCommand();
            command.CommandText = "dbo.uspGetIntegrationTypeById";
            command.CommandType = CommandType.StoredProcedure;
            ...
            objReturn = GetIntegrationTypeFromDataReader(result);

            return objReturn;

        }

        public async Task<IntegrationType> GetByNameAsync(string name, CancellationToken ct)
        {
            IntegrationType objReturn = new IntegrationType();

            // Ensure connection is open
            using var command = _context.Database.GetDbConnection().CreateCommand();
            command.CommandText = "dbo.uspGetIntegrationTypeByName";
            command.CommandType = CommandType.StoredProcedure;
        ...
        public async Task<Int64> CreateAsync(IntegrationTypeEntity industry, CancellationToken ct)
        {
            // Ensure connection is open
            using var command = _context.Database.GetDbConnection().CreateCommand();
            command.CommandText = "dbo.uspCreateIntegrationType";
            command.CommandType = CommandType.StoredProcedure;
        ...
        public async Task<Int64> UpdateAsync(IntegrationTypeEntity industry, CancellationToken ct)
        {
            // Ensure connection is open
            using var command = _context.Database.GetDbConnection().CreateCommand();
            command.CommandText = "dbo.uspUpdateIntegrationType";
            command.CommandType = CommandType.StoredProcedure;
        ...
        private IntegrationType GetIntegrationTypeFromDataReader(DbDataReader reader)
        {
            IntegrationType objReturn = new IntegrationType();
            if (reader.HasRows)
            {
        ...
        private List<IntegrationType> GetIntegrationTypeListFromDataReader(DbDataReader reader)
        {
            List<IntegrationType> lstReturn = new List<IntegrationType>();
            ...
            if (reader.HasRows)
            {
                while (reader.Read())
                {
                    IntegrationType newIndustry = new IntegrationType();
