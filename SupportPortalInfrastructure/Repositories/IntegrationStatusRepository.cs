...
{
    public class IntegrationStatusRepository : IGenericRepository<IntegrationStatus, IntegrationStatus, IntegrationStatusEntity>
    {
        ...
        public IntegrationStatusRepository(SupportPortalDBContext context)
        {
            _context = context ?? throw new ArgumentNullException(nameof(context));

        }

        public async Task<List<IntegrationStatus>> GetAllAsync(CancellationToken ct)
        {
            List<IntegrationStatus> lstReturn = new List<IntegrationStatus>();

            // Ensure connection is open
            using var command = _context.Database.GetDbConnection().CreateCommand();
            command.CommandText = "dbo.uspGetAllIntegrationStatuses";
            ...
            lstReturn = GetIntegrationStatusListFromDataReader(result);

            return lstReturn;

        }

        public async Task<List<IntegrationStatus>> GetAllActiveAsync(CancellationToken ct)
        {
            List<IntegrationStatus> lstReturn = new List<IntegrationStatus>();
        ...
        public Task<List<IntegrationStatus>> GetByParentIdAsync(Int64 parentId, CancellationToken ct)
        {
            throw new NotImplementedException("Parent property not valid for this object.");

        }

        public async Task<IntegrationStatus> GetByIdAsync(Int64 id, CancellationToken ct)
        {
            IntegrationStatus objReturn = new IntegrationStatus();

            // Ensure connection is open
            using var command = _context.Database.GetDbConnection().CreateCommand();
            command.CommandText = "dbo.uspGetIntegrationStatusById";
            command.CommandType = CommandType.StoredProcedure;
            ...
            objReturn = GetIntegrationStatusFromDataReader(result);

            return objReturn;

        }

        public async Task<IntegrationStatus> GetByNameAsync(string name, CancellationToken ct)
        {
            IntegrationStatus objReturn = new IntegrationStatus();

            // Ensure connection is open
            using var command = _context.Database.GetDbConnection().CreateCommand();
            command.CommandText = "dbo.uspGetINtegrationStatusByName";
            command.CommandType = CommandType.StoredProcedure;
        ...
        public async Task<Int64> CreateAsync(IntegrationStatusEntity industry, CancellationToken ct)
        {
            // Ensure connection is open
            using var command = _context.Database.GetDbConnection().CreateCommand();
            command.CommandText = "dbo.uspCreateIntegrationStatus";
            command.CommandType = CommandType.StoredProcedure;
        ...
        public async Task<Int64> UpdateAsync(IntegrationStatusEntity industry, CancellationToken ct)
        {
            // Ensure connection is open
            using var command = _context.Database.GetDbConnection().CreateCommand();
            command.CommandText = "dbo.uspUpdateIntegrationStatus";
            command.CommandType = CommandType.StoredProcedure;
        ...
        private IntegrationStatus GetIntegrationStatusFromDataReader(DbDataReader reader)
        {
            IntegrationStatus objReturn = new IntegrationStatus();
            if (reader.HasRows)
            {
        ...
        private List<IntegrationStatus> GetIntegrationStatusListFromDataReader(DbDataReader reader)
        {
            List<IntegrationStatus> lstReturn = new List<IntegrationStatus>();
            ...
            if (reader.HasRows)
            {
                while (reader.Read())
                {
                    IntegrationStatus newIndustry = new IntegrationStatus();
