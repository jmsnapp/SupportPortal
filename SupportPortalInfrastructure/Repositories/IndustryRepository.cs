...
{
    public class IndustryRepository : IGenericRepository<Industry, Industry, IndustryEntity>
    {
        ...
        public IndustryRepository(SupportPortalDBContext context)
        {
            _context = context ?? throw new ArgumentNullException(nameof(context));

        }

        public async Task<List<Industry>> GetAllAsync(CancellationToken ct) 
        {
            List<Industry> lstReturn = new List<Industry>();

            // Ensure connection is open
            using var command = _context.Database.GetDbConnection().CreateCommand();
            command.CommandText = "dbo.uspGetAllIndustries";
            ...
            lstReturn = GetIndustryListFromDataReader(result);

            return lstReturn;

        }

        public async Task<List<Industry>> GetAllActiveAsync(CancellationToken ct)
        {
            List<Industry> lstReturn = new List<Industry>();
        ...
        public Task<List<Industry>> GetByParentIdAsync(Int64 parentId, CancellationToken ct)
        {
            throw new NotImplementedException("Parent property not valid for this object.");

        }

        public async Task<Industry> GetByIdAsync(Int64 id, CancellationToken ct)
        {
            Industry objReturn = new Industry();

            // Ensure connection is open
            using var command = _context.Database.GetDbConnection().CreateCommand();
            command.CommandText = "dbo.uspGetIndustryById";
            command.CommandType = CommandType.StoredProcedure;
            ...
            objReturn = GetIndustryFromDataReader(result);

            return objReturn;

        }

        public async Task<Industry> GetByNameAsync(string name, CancellationToken ct)
        {
            Industry objReturn = new Industry();

            // Ensure connection is open
            using var command = _context.Database.GetDbConnection().CreateCommand();
            command.CommandText = "dbo.uspGetIndustryByName";
            command.CommandType = CommandType.StoredProcedure;
        ...
        public async Task<Int64> CreateAsync(IndustryEntity industry, CancellationToken ct)
        {
            // Ensure connection is open
            using var command = _context.Database.GetDbConnection().CreateCommand();
            command.CommandText = "dbo.uspCreateIndustry";
            command.CommandType = CommandType.StoredProcedure;
        ...
        public async Task<Int64> UpdateAsync(IndustryEntity industry, CancellationToken ct)
        {
            // Ensure connection is open
            using var command = _context.Database.GetDbConnection().CreateCommand();
            command.CommandText = "dbo.uspUpdateIndustry";
            command.CommandType = CommandType.StoredProcedure;
        ...
        private Industry GetIndustryFromDataReader(DbDataReader reader)
        {
            Industry objReturn = new Industry();
            if (reader.HasRows)
            {
        ...
        private List<Industry> GetIndustryListFromDataReader(DbDataReader reader)
        {
            List<Industry> lstReturn = new List<Industry>();
