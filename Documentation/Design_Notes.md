...
I WAS impressed with the value provided by Microsoft CoPilot in Visual Studio, as I am using the Visual Studio community edition, and am not paying for any extra tokens or resources.  In July of 2026, I used CoPilot for the process of scaffolding the database project, the infrastructure project, and the API. I did use up all my Inline suggestions, but less than 20% of my Monthly Limit for my copilot context.  Compare that to Anthropic's Claude, where I was not able to revise my resume using another version of the document and my CV without having to purchase their lowest tier service. However, this changed by September of 2026, when I used Copilot to regenerate all my unit tests after a significant architectural change, and used up my entire monthly allowance on September 1st. I am no longer impressed with Microsoft CoPilot. `<SARCASM>`Thank you, Microsoft!`</SARCASM>`
...
I am using Entity Framework Core as my ORM. I did this primarily to prove that I know how to use LINQ an ORM's in general and Entity Framework in particular. I effectively only use Entity Framework to manage the database connection, though.

In most of my roles, I have had sufficient access to the datastores that I could use stored procedures and other database layer code for CRUD and processing operations on the database. It has been my experience that database instances (whether on-premises or in the cloud) are usually better resourced, better maintained, and better monitored than application instances. Database vendors usually work VERY, VERY, hard to optimize their internal algorithms for maximum performance. As a result, there is a strong argument to be made to perform data processing in the database layer through database operations rather than up in the application layer using code or even a framework like LINQ.  Yes, LINQ and Entity Framework have become much more performant over the years, but that is mostly due to Microsoft using the algorithms in Entity Framework and LINQ that they developed and refined for MSSQL.

I do understand the attractions of ORMs, and I have used them on those rare occasions where my user account only had read access to the datastores and any changes had to be done through the application. In these situations, an ORM comes in extremely handy, as it will provide an object-based interface for data operations.

Yes, I also feel that LINQ has an important place in Microsoft based development. Any time you are dealing with data that you did not pull from a database layer yourself (say, from a third party API), it is very nice to have a SQL-like tool/methodology to parse and process that data.

What I did for this project is create a series of stored procedures for each table that handle the basic CRUD operations. This consists of:  
...
4. upsGetWhateverByName - returns a single row from the table that has that Name. Effectively `SELECT * FROM [Whatevers] WHERE [Name] = @value`. However, only tables that are commonly used for Lookups in the applications have a "Name" column.  It does not always make sense for some objects, so not every table wil have this procedure.
5.  uspGetWhateverByParentId - returns all the objects in the table that have the supplied parent Id.  This is intended to retrieve the common child objects of some other object. An example of this is retrieving all the Project Notes for a given Project. Like `upsGetWhateverByName`, this procedure may not be present for every table.
6. upsCreateWhatever - Inserts a new row in the `[Whatevers]` table.
7. uspUpdateWhatever - Updates the row in the `[Whatevers]` table. Because I use soft deleted by default, this also functionally acts as the Delete operation, just by setting the `[Deleted]` field to '1'.

## List Entities and List Objects

Many of the more complex concepts have two entities (and by extension two Models) that can represent them. The first is the actual entity and Model, which is used when dealing with a single instance of that concept. The model is fully hydrated, with all the object's children as fully hydrated objects themselves. The second type of entity or Model is a `<Whatever>`ListItem. These entities and models are intended to be used when a collection of the objects are returned or being processed. They will have all the base data type properties of the "full" version, but "Description" properties of their first order children instead of a fully realized child entity or object. The child entity or object's Id will be presented in a property if the developer needs to access it. Also, no child collections (like Project Notes or Ticket Notes) are provided. This allows the application developer to show whatever they feel is pertinent in a list of objects, but they still have the capability of accessing the child objects if they need to in the code.
