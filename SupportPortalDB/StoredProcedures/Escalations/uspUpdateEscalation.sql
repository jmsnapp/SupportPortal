CREATE PROCEDURE [dbo].[uspUpdateEscalation]
	@Id BIGINT,
...
AS
	...
	IF NOT EXISTS (SELECT 1 FROM [dbo].[Escalations]
				WHERE [Id] = @Id
	...
	-- chain of <> does not.
	UPDATE [dbo].[Escalations] SET
		[Description] = COALESCE(@Description, [Description]),
		[Deleted] = COALESCE(@Deleted, [Deleted]),
		[ProblemSummary] = COALESCE(@ProblemSummary, [ProblemSummary]),
		[CustomerImpact] = COALESCE(@CustomerImpact, [CustomerImpact]),
		[RootCause] = COALESCE(@RootCause, [RootCause]),
		[RecommendedActions] = COALESCE(@RecommendedActions, [RecommendedActions])
	WHERE [Id] = @Id
		  AND EXISTS (
				SELECT [Description],
				       [Deleted],
				...
				SELECT COALESCE(@Description, [Description]),
				       ...
				       COALESCE(@ProblemSummary, [ProblemSummary]),
				       COALESCE(@CustomerImpact, [CustomerImpact]),
				       COALESCE(@RootCause, [RootCause]),
				       COALESCE(@RecommendedActions, [RecommendedActions])
			  );
